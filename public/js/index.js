	...
	$('.swiper-slide').on('click', function(event) {
	    ...
	    ajaxAddCoupon(_this, function(data) {
	        ...
	        if (result.code == 0) {
	            ...
	            _this.find('.coupon-btn-new').addClass('YES').removeClass('NO');
	            _this.find('.coupon-btn-new').html("已领取,买单使用");
	        } else {
	            ...
	            if (results.code == 0) {
	                ...
	                if (pageData.length > 0) {
	                    ...
	                    for (var i in pageData) {
	                        ...
	                        str += '<a href="javascript:void(0)" ><div class="a-left"><span class="discount">' + amount + '</span><span class="couponType">' + type + '</span></div><div class="a-mid"><div class="name">' + pageData[i].userName + '</div><div class="tips">' + pageData[i].couponName + '</div></div><div class="a-right"><div class="a-right-btn ' + styClass + '" data-addr="' + pageData[i].userAddress + '" data-id="' + pageData[i].couponId + '">' + states + '</div><div class="a-right-endtime">效期至' + endTime + '</div></div></a>';
	                    }
